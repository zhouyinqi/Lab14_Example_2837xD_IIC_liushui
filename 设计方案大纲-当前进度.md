...
在 `Lab14_Example_2837xD_IIC_liushui` 内开发 F28377D DSP1 下位机测试程序，用于后续上位机一键触发开发板外设测试，并逐步接入 HPD 模组例行试验大纲中的可模拟项和试验台实测项。
...
| 工作区 | 有本轮开发变更和你的本地入口文件调整，未提交；`Debug/`、`Release/` 仍为忽略的编译输出 |
| Host 单元测试 | 通过 |
| Debug Flash 编译 | 通过 |
| Release Flash 编译 | 通过 |
...
flowchart TD
    ...
    E --> F{"command ?"}
    F -->|0 无命令| G["GPIO 心跳服务"]
    F -->|1 自动测试| H["BoardTest_StartAuto"]
    F -->|2 单项测试| I["BoardTest_StartSingle(testId)"]
...
| 框架 | 测试表、记录区、自动/单项执行 | 已完成 | 通过 CCS Expressions 写命令 |
| 命令接口 | `gBoardTestCommandMailbox` | 已完成 | 后续上位机协议会替换/扩展 |
...
| CAN-B | 内部回环收发 | 已开发 | Host/Debug/Release 已通过，待板上验证 |
...
| 优先级 | 模块 | 当前状态 | 下一步 |
|---|---|---|---|
| P0 | CAN-B | 已开发，待板上验证 | 先跑内部回环；板上通过后再决定是否做外部回环 |
...
| P1 | Ethernet/W5300 | 未实现 | DSP1 侧需确认与 DSP2 管脚边界 |
...
```text
第一步：软件模拟
- 能用软件生成的目标值、上下限、判断逻辑先实现。
- 不依赖真实功率输出，不打开驱动。
- 结果写入统一 records[]。

第二步：试验台实测
- 试验台搭建完成后，再接 ADC/PWM/CAN/外部传感器。
- 高功率项默认不上自动测试，必须手动授权。
- 每个实测项必须有安全前置条件、超时、错误码。
```
...
下一步先下载 Debug Flash `.out` 验证 `CAN-B` 内部回环。板上通过后，再开发 SPI-A/B/C 基础自检；之后进入 ADC/PWM/HPD 软件模拟和安全授权框架。
...
- gBoardCanLoopbackSnapshot.statusMask = 0x001F
