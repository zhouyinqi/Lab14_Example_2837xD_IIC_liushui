    ...
    BoardTest_U16 *responseLength)
{
    ...
    BoardTest_U16 status;
    BoardTest_U16 pwmChannel;
    BoardTest_U16 singleSelection;
    const BoardTest_Record *record;
    const BoardTest_Descriptor *descriptor;
    BoardTest_Record availabilityRecord;
    ...
    record = 0;
    descriptor = 0;
    availabilityRecord.id = BOARD_TEST_INVALID_ID;
    availabilityRecord.result = BOARD_PROFILE_TEST_NOT_AVAILABLE;
    availabilityRecord.errorCode = 0U;
    availabilityRecord.rawValue = 0UL;
    availabilityRecord.measuredValue = 0.0F;
    availabilityRecord.expectedMin = 0.0F;
    availabilityRecord.expectedMax = 0.0F;
    ...
    pwmChannel = (BoardTest_U16)
        ((flags & BOARD_HOST_PROTOCOL_FLAG_PWM_CHANNEL_MASK) >>
         BOARD_HOST_PROTOCOL_FLAG_PWM_CHANNEL_SHIFT);
    singleSelection = (BoardTest_U16)
        ((flags & BOARD_HOST_PROTOCOL_FLAG_SINGLE_SELECTION_MASK) >>
         BOARD_HOST_PROTOCOL_FLAG_SINGLE_SELECTION_SHIFT);
    ...
    else if(opcode == BOARD_HOST_PROTOCOL_COMMAND_GET_TEST_AVAILABILITY)
    {
        descriptor = BoardTest_GetDescriptorById(argument);
        if(descriptor == 0)
        {
            status = BOARD_HOST_PROTOCOL_STATUS_TEST_ID;
        }
        else
        {
            availabilityRecord.id = descriptor->id;
            availabilityRecord.result = (BoardTest_U16)
                BoardProfile_GetTestAvailability(
                    descriptor->id,
                    descriptor->capabilities);
            availabilityRecord.errorCode = descriptor->capabilities;
            availabilityRecord.rawValue =
                ((BoardTest_U32)descriptor->stageMask << 16U) |
                (BoardTest_U32)descriptor->risk;
            availabilityRecord.measuredValue =
                (float)descriptor->enabledInAuto;
            record = &availabilityRecord;
            status = BOARD_HOST_PROTOCOL_STATUS_OK;
        }
    }
    else if(opcode == BOARD_HOST_PROTOCOL_COMMAND_CONFIGURE_ADC_INJECTION)
    {
        ...
        else if(BoardAdc_ConfigureExternalInjection(
                    stage,
                    argument,
                    (BoardTest_U16)flags *
                        BOARD_HOST_PROTOCOL_ADC_TOLERANCE_STEP_MV) == 0U)
        {
            ...
            (opcode == BOARD_HOST_PROTOCOL_COMMAND_START_SINGLE))
    {
        ...
        else if((argument == BOARD_TEST_ID_PWM_DIDO_EXTERNAL) &&
                (pwmChannel >= BOARD_PWM_DIDO_OUTPUT_COUNT))
        {
            status = BOARD_HOST_PROTOCOL_STATUS_ARGUMENT;
        }
        else if((argument == BOARD_TEST_ID_DRIVER_FAULT_DI_EXTERNAL) &&
                (singleSelection != BOARD_DI_DRIVER_FAULT_GROUP_1_MASK) &&
                (singleSelection != BOARD_DI_DRIVER_FAULT_GROUP_2_MASK))
        {
            status = BOARD_HOST_PROTOCOL_STATUS_ARGUMENT;
        }
        else if((argument == BOARD_TEST_ID_PROTECTION_FAULT_DI_EXTERNAL) &&
                (BoardAdc_IsProtectionSelectionSupported(
                     singleSelection) == 0U))
        {
            status = BOARD_HOST_PROTOCOL_STATUS_ARGUMENT;
        }
        else if((argument == BOARD_TEST_ID_DRIVER_RESET_DO_EXTERNAL) &&
                (singleSelection >= BOARD_PROFILE_DRIVER_RESET_DO_COUNT))
        {
            status = BOARD_HOST_PROTOCOL_STATUS_ARGUMENT;
        }
        ...
        else
        {
            ...
            {
                gBoardTestCommandMailbox.pwmDidoTestArmKey =
                    BOARD_PWM_DIDO_TEST_ARM_KEY;
                gBoardTestCommandMailbox.pwmDidoOutputMask =
                    (BoardTest_U16)(1U << pwmChannel);
            }
            else if(argument == BOARD_TEST_ID_DRIVER_FAULT_DI_EXTERNAL)
            {
                gBoardTestCommandMailbox.driverFaultDiChannelMask =
                    singleSelection;
            }
            else if(argument == BOARD_TEST_ID_PROTECTION_FAULT_DI_EXTERNAL)
            {
                gBoardTestCommandMailbox.protectionSelection =
                    singleSelection;
            }
            else if(argument == BOARD_TEST_ID_DRIVER_RESET_DO_EXTERNAL)
            {
                gBoardTestCommandMailbox.driverResetTestArmKey =
                    BOARD_DIDO_TEST_ARM_KEY;
                gBoardTestCommandMailbox.driverResetOutputMask =
                    (BoardTest_U16)(1U << singleSelection);
            }
            gBoardTestCommandMailbox.command =
...
static BoardTest_Result BoardTest_TargetRunDoExternal(
    BoardTest_Record *record)
{
    ...
    if((capabilities & BOARD_PROFILE_CAP_FPGA_DIDO) != 0UL)
    {
    ...
    else if((capabilities & BOARD_PROFILE_CAP_DSP_DO) != 0UL)
    {
        result = BoardDido_RunDspDoExternalTest(
            record,
            armKey,
            channelMask);
    }
...
static BoardTest_Result BoardTest_TargetRunPwmDidoExternal(
    BoardTest_Record *record)
{
    BoardTest_Result result;

    result = BoardPwm_RunDidoExternalTest(
        record,
        gBoardTestCommandMailbox.pwmDidoTestArmKey,
        gBoardTestCommandMailbox.pwmDidoOutputMask);
    if(result != BOARD_TEST_RESULT_RUNNING)
    {
        ...
        gBoardTestCommandMailbox.pwmDidoOutputMask = 0U;
    }
    return result;
}

static BoardTest_Result BoardTest_TargetRunDiExternal(
    BoardTest_Record *record)
{
    BoardTest_U32 capabilities;
    BoardTest_U16 channelMask;
    ...
    if(gBoardTestManager.mode == BOARD_TEST_MODE_AUTO)
    {
        channelMask = ((capabilities & BOARD_PROFILE_CAP_DSP_DI) != 0UL) ?
            BOARD_DI_DSP_DEFAULT_CHANNEL_MASK : BOARD_DI_DEFAULT_CHANNEL_MASK;
    }

    if((capabilities & BOARD_PROFILE_CAP_FPGA_DIDO) != 0UL)
    {
        return BoardDi_RunFpgaExternalTest(record, channelMask);
    }
    if((capabilities & BOARD_PROFILE_CAP_DSP_DI) != 0UL)
    {
        return BoardDi_RunDspExternalTest(
            record,
                                         ...
                                         BoardTest_Record *record)
{
    switch(testId)
    {
        ...
        case BOARD_TEST_ID_ADC_EXTERNAL_INJECTION:
            return BoardAdc_RunExternalInjectionTest(record);

        case BOARD_TEST_ID_ADC_EXTERNAL_GROUP_1:
            return BoardAdc_RunExternalInjectionGroupTest(
                BOARD_ADC_EXTERNAL_SELECTION_GROUP_1,
                record);

        case BOARD_TEST_ID_ADC_EXTERNAL_GROUP_2:
            return BoardAdc_RunExternalInjectionGroupTest(
                BOARD_ADC_EXTERNAL_SELECTION_GROUP_2,
                record);

        case BOARD_TEST_ID_ADC_PT100_MUX_EXTERNAL:
        case BOARD_TEST_ID_ADC_FAULT_MUX_EXTERNAL:
            return BoardAdc_RunMuxInjectionTest(testId, record);

        case BOARD_TEST_ID_PWM_BASIC:
            return BoardPwm_RunSafetyConfigTest(record);

        case BOARD_TEST_ID_PWM_DIDO_EXTERNAL:
            return BoardTest_TargetRunPwmDidoExternal(record);
        ...
        case BOARD_TEST_ID_DIDO_FPGA_DO_EXTERNAL:
            return BoardTest_TargetRunDoExternal(record);
        ...
        case BOARD_TEST_ID_DRIVER_FAULT_DI_EXTERNAL:
        {
            ...
            channelMask = (gBoardTestManager.mode == BOARD_TEST_MODE_MANUAL) ?
                gBoardTestCommandMailbox.driverFaultDiChannelMask :
                BOARD_DI_DRIVER_FAULT_DEFAULT_CHANNEL_MASK;
            result = BoardDi_RunDriverFaultExternalTest(record, channelMask);
            gBoardTestCommandMailbox.driverFaultDiChannelMask = 0U;
            return result;
        }

        case BOARD_TEST_ID_PROTECTION_FAULT_DI_EXTERNAL:
            return BoardAdc_RunProtectionThresholdTest(
                gBoardTestCommandMailbox.protectionSelection,
                record);

        case BOARD_TEST_ID_DRIVER_RESET_DO_EXTERNAL:
        {
            BoardTest_Result result;

            result = BoardDido_RunDriverResetExternalTest(
                record,
                gBoardTestCommandMailbox.driverResetTestArmKey,
                gBoardTestCommandMailbox.driverResetOutputMask);
            if(result != BOARD_TEST_RESULT_RUNNING)
            {
                gBoardTestCommandMailbox.driverResetTestArmKey = 0U;
                gBoardTestCommandMailbox.driverResetOutputMask = 0U;
            }
            return result;
        }
...
void BoardTest_TargetPoll(void)
{
    ...
    else if(command == BOARD_TEST_COMMAND_STOP)
    {
        ...
        gBoardTestCommandMailbox.pwmDidoOutputMask = 0U;
        gBoardTestCommandMailbox.driverFaultDiChannelMask = 0U;
        gBoardTestCommandMailbox.protectionSelection = 0U;
        BoardAdc_AbortProtectionThresholdTest();
        gBoardTestCommandMailbox.driverResetTestArmKey = 0U;
        gBoardTestCommandMailbox.driverResetOutputMask = 0U;
        BoardDido_ForceDspDoSafe();
        BoardDido_ForceDriverResetSafe();
        BoardPwm_ForceDidoSafe();
        BoardTest_TargetStopCommunicationStandby();
        BoardTest_Stop();
