...
- 用户确认先完成核心板基础、通信/存储、ADC/PWM安全配置，再进行外部通信和0x0400～0x040A；软件通过即继续，实板状态单独记录。当前已建立 LOW_VOLTAGE_V04_REVIEW.md 覆盖表，核心基础修正完成，EMIF/W5300总线隔离已完成并通过主机测试/CCS构建；CAN/SCI/SPI已复核，SCIA V04路由与Qt单项收发已适配，ADC13基础扫描和PWM安全配置已通过DSP回归及CCS构建；0x0400 DI软件已完成并通过DSP/CCS/Qt复核；下一项核查0x0401/0402输出依据，并推进0x0403 STO只读输入。
