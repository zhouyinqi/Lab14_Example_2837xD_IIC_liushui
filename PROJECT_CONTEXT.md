...
- 低压 V04 使用 F28377D + EMIF W5300；能力与管脚矩阵、专用 ID 0x0400～0x040A 已建，DI/STO专用输入测试已实现，其他专用测试待开发。
- 用户确认先完成核心板基础、通信/存储、ADC/PWM安全配置，再进行外部通信和0x0400～0x040A；软件通过即继续，实板状态单独记录。当前已建立 LOW_VOLTAGE_V04_REVIEW.md 覆盖表，核心基础修正完成，EMIF/W5300总线隔离已完成并通过主机测试/CCS构建；CAN/SCI/SPI已复核，SCIA V04路由与Qt单项收发已适配，ADC13基础扫描和PWM安全配置已通过DSP回归及CCS构建；0x0400 DI软件已完成并通过DSP/CCS/Qt复核；0x0403 STO只读输入及Qt模拟协议完成通过；下一项复核板型隔离和其余接口硬件依据。
- 后续顺序：0x0401 DO/软启动/风扇、0x0402 HDO、0x0403 STO已完成软件；其余 ADC、PT100、NTC、PWM、保护、AO、旋变仍待开发。
...
- Windows，CCS DSP 工程；Qt 6.11.1 MinGW 64 位，MinGW 13.1，CMake。DSP编译器已核实为TI C2000 18.1.4.LTS；当前烧录器型号待核对。
...
- 本项Qt界面补充了配置检查边界，HpdHostTests构建及ctest 1/1通过；未烧录。
`n- V04旧双驱动夹具ID已隔离；DSP回归、CCS全量构建与Qt程序/测试构建通过。下一项补充两路RS485断联自主超时与资源恢复。
